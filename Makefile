...
.PHONY: yarn-setup
yarn-setup: yarn.lock .yarnrc.yml ## Make sure that this directory can be used as a yarn workspace
...
.PHONY: clean
clean: ## Remove all generated javascript files
	if [ -d dist ]; then rm -rf dist; fi
	for f in .tsbuildinfo tsconfig*tsbuildinfo; do if [ -f $$f ]; then rm $$f; fi; done
